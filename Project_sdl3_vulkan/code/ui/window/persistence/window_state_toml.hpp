...
struct WindowStateToml
{
    ...
    struct ImageHistoryEntry
    {
        ...
        bool hwdec_enabled = false;  // recreate mpv with hardware decode enabled for this entry
        int resume_position_seconds = 0; // playback position restored when reopening this entry
        bool startup_restore = false; // reopen this entry automatically on next startup
    };
